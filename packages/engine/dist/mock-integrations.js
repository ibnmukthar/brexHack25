...
class MockIntegrations {
    ...
    // 3PL Provider APIs
    async query3PL(provider, service, params = {}) {
        ...
        const serviceData = providerData.services[service];
        if (!serviceData) {
    ...
    async queryFreightForwarder(forwarder, shipmentType, params = {}) {
        ...
        const forwarderData = forwarders[forwarder];
        if (!forwarderData) {
            return { error: `Unknown freight forwarder: ${forwarder}` };
        }
        const serviceData = forwarderData.services[shipmentType];
        if (!serviceData) {
    ...
    async queryPortStatus(portCode) {
        await this.delay(500);
        const ports = {
            'USLAX': {
    ...
    async queryWMS(operation, params = {}) {
        await this.delay(400);
        const operations = {
            'inventory_levels': {
                locations: [
    ...
    async queryTMS(function_name, params = {}) {
        await this.delay(600);
        const functions = {
            'load_planning': {
                available_capacity: {
        ...
        const functionData = functions[function_name];
        if (!functionData) {
